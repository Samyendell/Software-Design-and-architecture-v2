...
/**
 * Fixed: Each variation runs independently, not combined.
 */
public class PlayRunner {
    ...
    public void runSingleDieVariation() {
        ...
        // FIXED: Only single die variation, others false
        GameConfiguration config = new GameConfiguration(
                List.of(PlayerColour.RED, PlayerColour.BLUE),
                true,   // useSingleDie = TRUE
                false,  // exactLandingRequired = FALSE
                false,  // forfeitOnHit = FALSE
                false
        );
        observer.setConfiguration(config);
    ...
    public void runExactLandingVariation() {
        System.out.println("=".repeat(60));
        System.out.println("VARIATION: EXACT LANDING");
        System.out.println("=".repeat(60));

        // FIXED: Only exact landing variation, others false
        GameConfiguration config = new GameConfiguration(
                List.of(PlayerColour.RED, PlayerColour.BLUE),
                false,  // useSingleDie = FALSE
                true,   // exactLandingRequired = TRUE
                false,  // forfeitOnHit = FALSE
                false
        );
        observer.setConfiguration(config);
    ...
    public void runForfeitOnHitVariation() {
        ...
        // FIXED: Only forfeit on hit variation, others false
        GameConfiguration config = new GameConfiguration(
                List.of(PlayerColour.RED, PlayerColour.BLUE),
                false,  // useSingleDie = FALSE
                false,  // exactLandingRequired = FALSE (not true!)
                true,   // forfeitOnHit = TRUE
                false
        );
        observer.setConfiguration(config);
