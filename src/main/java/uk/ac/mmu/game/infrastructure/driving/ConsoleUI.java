...
public class ConsoleUI {
    ...
    public void showReplayMenu() {
        ...
        System.out.println("GAME REPLAY MENU");
        System.out.println("=".repeat(60));
        System.out.println();
        ...
        List<GameId> gameIds = repository.listAll();
        ...
        for (int i = 0; i < gameIds.size(); i++) {
            ...
            System.out.printf("%d. Game ID: %s%n", i + 1, gameId.getValue().substring(0, 8) + "...");

            repository.findById(gameId).ifPresent(record -> {
                System.out.printf("   Winner: %s in %d turns (Total: %d turns)%n", record.getWinner(),
                        record.getWinnerTurns(), record.getTotalTurns());
        ...
        try {
            ...
            if (selection < 1 || selection > gameIds.size()) {
                System.out.println("Invalid selection. Please choose a number between 1 and " + gameIds.size());
                return;
            }
